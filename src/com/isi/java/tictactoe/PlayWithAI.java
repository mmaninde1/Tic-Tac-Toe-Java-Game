...
public class PlayWithAI extends JFrame implements ActionListener
{
	...
	public PlayWithAI()
	{
		super("Tic Tac Toe Game: Play with AI");
	...
	private void createComponents()
	{
		for (int i = 0; i < allButton.length; i++) {
			...
			allButton[i].setText("");
			allButton[i].setFont(new Font("Arial", Font.PLAIN, 40));
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		// create runnable thread and call computer turn function
		Runnable task = () -> {
			computerTurn();
		};
		// start the thread
	...
	{
		do {
			player1Name = JOptionPane.showInputDialog("Enter Name of Player: ");
	...
	private void computerTurn()
	{
		...
		if(checkZeroCross%2!=0)
		{
			...
			do {
				...
				if(allButton[x].isEnabled())
				{
					allButton[x].setText("O");
					allButton[x].setEnabled(false);
					chec = false;
				}
			} while (chec);
			...
			if(checkWinOrNot())
			{
				String text = allButton[x].getText();
				if(text == "X")
				{
					xWins++;
					text = player1Name + " Wins";	
